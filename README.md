# Overview

Pokédex, a high technical device, designed to record and store data on all species of Pokémon.
In this web application, the user can search up their favorite Pokémon and observe their abilities, levels and evolution!
This website was built with pure HTML, CSS and Vanilla JavaScript for practice.

# Features

The following features that Pokédex offers:

- Search for a Pokémon by name
- View their height & weight
- View their levels
- Explore their abilities
- Explore their evolution
